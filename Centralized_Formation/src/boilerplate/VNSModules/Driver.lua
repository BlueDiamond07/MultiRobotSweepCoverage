...
Mytimer = 0	-- a timer during which the supervisor UAV waits for 200 steps when a border is detected, then shifts for a certain period depending on the length of the formation, and then waits for 100 steps after compeleting the shift (see line 233)
...
function Driver:run(vns, paraT)
		...
		vns.normal = 0	-- this flag is used for obstacle avoidance when the supervisor UAV is shifting to left or right
		...
		if Mytimer > 0 then
			...
			elseif Mytimer == 1 then
				...
				for _, child in pairs(vns.childrenTVns) do
					-- the movement direction of the supervisor UAV is sent to the ground robots
		...
		-- add random speed
		-- this speed makes the supervisor UAV move forward (if vns.reverseMove is equal to 0) or backward (if vns.reverseMove is equal to 1) 
		...
		-- if a ground robot receives normal message, it sets its normal variable to 1; as already mentioned, this flag is used for obstacle avoidance when the supervisor UAV is shifting to left or right
		...
		for _, msgM in ipairs(vns.Msg.getAM("ALLMSG", "avoid")) do
	        ...
	        -- the first 1000 steps is used to form the formation; The supervisor UAV starts moving at step 1001 and the experimental data is recorded onward
		...
		-- lines 231 to 251 simulate a boustrophedon path for the supervisor UAV:
			-- the supervisor UAV stops for 200 steps as soon as detecting a border in its front or back (i.e., after sweeping a lane) to let the ground robots that are behind reach the group
			-- then, the supervisor UAV for 714 time steps shifts; afterwards, it stays stationary for 100 steps
			-- next, the supervisor UAV reverses the direction of its movement: from forward (i.e., vns.reverseMove == 0) to backward (i.e., vns.reverseMove == 1) or from backward to forward
		...
		if flag == 0 and lock == 0 then	-- the following function is called by the brain in order to move forward or backward (depending on the movement direction)
		...
		for _, robotVns in pairs(vns.childrenTVns) do -- for all children
			...
			-- executed by suepvisor UAV: define a temporary  x coordinate for the target point that is closer to the ground robot compared to the x coordinate of the actual target point: to help the robot get back to the approximately middle of its lane faster if it is not there
			...
			-- not used in this project the non-brain UAVs are not asked to move when their distance from the target point is less than 1 cm (in the view sight of the parent UAV) 
			...
			for _, robotVns2 in pairs(paraT.vehiclesTR) do	-- for all the ground robots that can be seen
				if robotVns.idS ~= robotVns2.idS then
					...
					if disTo < 5.7 then   -- this is equivalent to 3 cm distance in the simulation in practice
						if (vns.reverseMove == 0) then
