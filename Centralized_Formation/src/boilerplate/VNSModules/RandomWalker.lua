...
function RandomWalker:run(vns, paraT)
	...
	local z = 0
	if vns.robotType == "quadcopter" and paraT.stepCounter<1000 then -- make the supervisor UAV stay stationary in the first 1000 steps (i.e., during the initilization phase)
		x = 0
		y = 0
	elseif vns.robotType == "quadcopter" then
		if vns.reverseMove == 0 then  -- make the supervisor UAV move forward
			x = 30
			y = 0
		else  -- make the supervisor UAV move backward
			x = -30
			y = 0
		end
