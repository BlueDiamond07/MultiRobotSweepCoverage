...
namespace argos {
	...
	void CBoilerplateLoopFunctions::Init(TConfigurationNode& t_tree) {
		sum = 0; // total number of messages passed within the swarm (i.e., including the UAV)
		totalDis = 0;// total distance travelled by the swarm of ground robots (i.e., excluding the UAV)
		temp = 0;
		srand(time(0));
	...
	void CBoilerplateLoopFunctions::PreStep() {
		if (simcount==0){
			...
			for(CSpace::TMapPerType::iterator it = cFBMap.begin(); it != cFBMap.end(); ++it) {
				...
				float randX;
				float randY;
				// the following if statement is used to distirbute robots in a region below the UAV in the initialization phase so that they do not collide with each other or any obstacle
				if (cFB->HasComponent("tags")){
					robot_distribution = 0;
					if (simcount == 0){
						while (robot_distribution == 0){
							randY = ((float)rand() / RAND_MAX) * (-2 + 2.5) - 2.5;
							randX = ((float)rand() / RAND_MAX) * (0.1 - 1.9) + 1.9;
							int checkRand = 0;
							for(j=0;j<Nb_blocks;j++){ // when the robots are distributed near to UAV the distance between a robot and any obstacle should be more than 10 cm
								temp3 = sqrt(pow((xCoordinates[j]-randX),2)+pow((yCoordinates[j]-randY),2));
								if(temp3<0.1){
									checkRand = 1;
								} 
							}
							for(j=0;j<Nb_robots;j++){ // when the robots are distributed near to UAV the distance between each pair of robots should be more than 10 cm
   ...
   /****************************************/
	void CBoilerplateLoopFunctions::PostStep() {
		...
		for(CSpace::TMapPerType::iterator it = cFBMap.begin(); it != cFBMap.end(); ++it) {
			/* Create a reference for easier coding */
			CPrototypeEntity* cFB = any_cast<CPrototypeEntity*>(it->second); // create a reference to the obstacles and the robots for easier coding
			// note: the first 1000 steps of an experiment is used for the initialization phase where the formation is completely formed		
			...
			if (cFB->HasComponent("tags") && simcount>1000){ // for each robot after time step 1000				
				...
				if (c>=0){ 
					...
					// the following if-elseif statement (lines 380 to 487) is used to allow the system (formation) enters the main field after the initialization phase (from time step 100 to 1300), and
					// exits the main field when it is about to finish the sweep task, close to a border (after time step 4760)
					// also when a robot is disconnected form the system, it changes its direction randomly at the borders (as explained in the paper) based on the mentioned if-elseif
					if (simcount < 1300){
					}
