...
namespace argos {
   ...
   /****************************************/
	...
	void CBoilerplateLoopFunctions::Init(TConfigurationNode& t_tree) {
		sum = 0; // total number of messages passed within the swarm (i.e., including the UAVs)
		totalDis = 0; // total distance travelled by the swarm of ground robots (i.e., excluding the UAVs)
		temp = 0;
		srand(time(0));
		...
		m_strOutFile3 = str + "/boilerplate.argos"; // record the xml file for the environmental setups
		...
		m_strOutFile10 = str + "/blocks.csv"; // the csv file which contains the obstacles' position and orentation information
   ...
   /****************************************/
	bool CBoilerplateLoopFunctions::IsExperimentFinished() {
		if (simcount == 5101){ // termination time step
			return true;		
		}
		return false;
   ...
   /****************************************/
	void CBoilerplateLoopFunctions::PreStep() {
		if (simcount==0){
			...
			a = cArenaSize[0]-6; // a fraction of the length of the main field's border along x axis (i.e., length_of_the_border/4)
			b = cArenaSize[1]-6; // a fraction of the length of the main field's border along y axis (i.e., length_of_the_border/4)
			...
			m_cOutFile10.close();
			// read the blocks.csv file and based on it place the obstacles in the environment: lines  225 to 258
			int cc = 0;
			std::ifstream classFile("blocks.csv"); // the csv file which contains the obstacles' position and orentation information
			std::string line;
			...
			for(CSpace::TMapPerType::iterator it = cFBMap.begin(); it != cFBMap.end(); ++it) {
				...
				float randX;
				float randY;
				// the following if statement is used to distirbute robots in a region below the UAVs in the initialization phase so that they do not collide with each other or any obstacle
				if (cFB->HasComponent("tags")){
					robot_distribution = 0;
					if (simcount == 0){
						while (robot_distribution == 0){
							randY = ((float)rand() / RAND_MAX) * (-1.9 + 2.5) - 2.5;
							randX = ((float)rand() / RAND_MAX) * (0.1 - 1.9) + 1.9;
							int checkRand = 0;
							for(j=0;j<Nb_blocks;j++){ // when the robots are distributed near to UAVs the distance between a robot and any obstacle should be more than 10 cm
								temp3 = sqrt(pow((xCoordinates[j]-randX),2)+pow((yCoordinates[j]-randY),2));
								if(temp3<0.1){
									checkRand = 1;
								} 
							}
							for(j=0;j<Nb_robots;j++){ // when the robots are distributed near to UAVs the distance between each pair of robots should be more than 10 cm
   ...
   /****************************************/
	void CBoilerplateLoopFunctions::PostStep() {
		...
		for(CSpace::TMapPerType::iterator it = cFBMap.begin(); it != cFBMap.end(); ++it) {
			/* Create a reference for easier coding */
			CPrototypeEntity* cFB = any_cast<CPrototypeEntity*>(it->second); // create a reference to the obstacles and the robots for easier coding
			// note: the first 1000 steps of an experiment is used for the initialization phase where the MNS is completely formed
			// after time step 1000 the data of the experiment is recorded as we do not study the initialization phase; time step 1001 is counted as 1, time step 1002 is counted as 2, and so on 
			// calculate and record the number of messages sent by a robot/UAV in the current step (after the initialization phase) and
			// the total number of messages sent within the swarm: lines: 331 to 339
			if (cFB->HasControllableEntity() && simcount>1000){
			...
			// record the position of the robots in a vector right after the initialization phase (i.e., time step 1000) in order to calculate the total distance travelled by them: lines 341 to 345
			if (cFB->HasComponent("tags") && simcount==1000){
			...
			if (cFB->HasComponent("tags") && simcount>1000){ // for each robot after time step 1000		

				if (c>=0){
					// to keep the faulty robot stationary from time step 1500 (which is recorded as time step 500); lines: 350 to 373
					int failureFlag = 0;
					for (int i=0;i<6;i++){
						if(simcount >= 1500 && cFB->GetId() == IDs[i]){
							failureFlag = 1;
						}
					}
					if (failureFlag == 1){
					...
					// the following if-elseif statement (lines 380 to 487) is used to allow the MNS enters the main field after the initialization phase (from time step 100 to 1300), and
					// exits the main field when it is about to finish the sweep task, close to a border (after time step 4760)
					// also when a robot is disconnected form the MNS, it changes its direction randomly at the borders (as explained in the paper) based on the mentioned if-elseif
					if (simcount < 1300){
					}
					else if (simcount > 4760){
						if (cFB->GetEmbodiedEntity().GetOriginAnchor().Position[1]<-2 && cFB->GetEmbodiedEntity().GetOriginAnchor().Position[0]>0){
						v = rand() % 2;
						if (v == 0) {
							v1 = rand() % 180;
		...
		}//end of the for loop of UAVs, robot, and obstacle space
		...
		if (simcount>1000){
			...
			double new_uniformity = std::sqrt(Median_div_visited_p1); //the output of the formula (first calculate the summation (line 568), then divide it by #visited cells, and finally calculate its second root)
			//-------------------------------------------------------------------------------------
			//-------------------------------------------------------------------------------------
			// record the data
			...
			<< sum << "\t"
			<< sumOfCollisions << "\t"
			<< (256-zeroCounter)*100/256.0 << "\t" 
			<< 256-zeroCounter << "\t"
			<< zeroCounter << "\t"
			<< new_uniformity << "\t"
			<< GetSpace().GetSimulationClock()-1000 // time step (counted right after the initialization phase which takes 1000 steps)
			<< std::endl;
		...
		}// end of if  (simcount>1000) 
		
		if (simcount==5101){ // at the termination time step
