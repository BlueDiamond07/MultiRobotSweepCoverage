...
VNS.move = function(transV3, rotateV3,stepCounter, randomflag)
	-- a flag (which is set at line 156) used to manage the rotation of the robot when it receives two negative values from the UAV as its target left wheel speed and right wheel speed
	if flagI>0 then
		flagI = flagI - 1
	end
	...
	---------------------------------------------------------------
	if vns.parentS == nil then	-- if a robot is not connected to any UAV
		...
		if (fault_flag == 1) then	-- if the robot is faulty, set the speed to 0  
			...
			-- to this end, when both left wheel and right wheel speeds sent by a parent UAV are negative, the ground robot rotates for 7 steps to adjust its heading with respect to the heading of the parent UAV
			...
			if flagI > 0 then
				...
				if right < 0 and left < 0 then
					if stepCounter > 1000 then
						...
						-- the ground robot rotates to adjust its heading with respect to the heading of the parent UAV
						if vns.flagTurn == 0 then
