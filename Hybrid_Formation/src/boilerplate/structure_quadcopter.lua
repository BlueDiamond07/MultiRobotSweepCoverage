...
function init()
	...
	----------------------------------------
	local dis = 30
        -- the following structrue is used in case of using 3 UAVs and 8 ground robots
	local structure = {
		locV3 = Vec3:create(),
		...
		children = {
			...
			{	robotType = "vehicle",
				locV3 = Vec3:create(0*dis, 3.75*dis, 0),
				dirQ = Quaternion:create(0,0,1, 0),
			},
			{	robotType = "vehicle",
				locV3 = Vec3:create(0*dis, -3.75*dis, 0),
			...
			{	robotType = "quadcopter",
				locV3 = Vec3:create(0, -210, 0),
				dirQ = Quaternion:create(0,0,1, 0),
				children = {
					{	robotType = "vehicle",
						locV3 = Vec3:create(0*dis,1.0*dis, 0),
						dirQ = Quaternion:create(0,0,1, 0),
					},

					{	robotType = "vehicle",
						locV3 = Vec3:create(0*dis,-1.5*dis, 0),
						dirQ = Quaternion:create(0,0,1, 0),
					},
				},
			},					
			...
			{	robotType = "quadcopter",
				...
				children = {
					...
					{	robotType = "vehicle",
						locV3 = Vec3:create(0*dis,-1*dis, 0),
						dirQ = Quaternion:create(0,0,1, 0),
					},
				},
			},			
...
--   Customize Functions
------------------------------------------------------------------------
function detectCorner()	-- this function is used for detecting the border from front of the MNS or from back of the MNS
	local cornerflag = 0
	for i, detectionT in ipairs(IF.getLEDsT()) do
		-- not used in this project: for detecting a white LED in the specified range from the brain (brain's camera)
		...
		elseif (vns.parentS == nil  and detectionT.color.green == 255 and detectionT.color.red == 255 and detectionT.center.x>=395 and detectionT.center.x<=400) then 
			cornerflag = 1
		-- for detecting a border (yellow LEDs) from the back of the MNS, in the specified range from the brain (brain's camera)
		elseif (stepCounter > 1500 and vns.parentS == nil  and detectionT.color.green == 255 and detectionT.color.red == 255 and detectionT.center.x>=230 and detectionT.center.x<=258) then
			cornerflag = 2
		end		
	end	
	return cornerflag
...
-- this function is called during the time that the MNS is shifting or during the time that the MNS is waiting after finishing a shift process along a border
...
function IF.setVelocity(x, y, w)
	-- to make sure all the non brain UAVs aslo stay stationary in the last 190 steps to avoid unnecessary fluctuations 
	-- (the brain definitely stays stationary for the last 200 steps as it detects the boundary):
	if stepCounter >= 4912 then
		x = 0
		y = 0
	end
